...
public class FramingTest
{
    ...
    private static SequenceOfFrames sequenceOfFrames(Random random, FrameEncoder encoder)
    {
        ...
        ByteBuffer frames = BufferPoolManager.temporary().getAtLeast(cumulativeCompressedLength[frameCount - 1], BufferType.OFF_HEAP);
    ...
    private static SequenceOfFrames sequenceOfMessages(Random random, float largeRatio, int messagingVersion)
    {
        ...
        ByteBuffer frames = BufferPoolManager.temporary().getAtLeast(cumulativeLength[messageCount - 1], BufferType.OFF_HEAP);
