...
public class NativeIndexDDLTest extends SAITester
{
    ...
    public void shouldIndexExistingMemtableOnCreationWithConcurrentFlush() throws Throwable
    {
        ...
        assertEquals("Segment memory limiter should revert to zero.", 0L, getSegmentBufferUsedBytes(jmxConnection));
    ...
    public void shouldReleaseIndexFilesAfterDroppingLastIndex() throws Throwable
    {
        ...
        assertEquals("Segment memory limiter should revert to zero on drop.", 0L, getSegmentBufferUsedBytes(jmxConnection));
        assertEquals("There should be no segment builders in progress.", 0L, getColumnIndexBuildsInProgress(jmxConnection));
    }
    ...
    public void shouldReleaseIndexFilesAfterCompaction() throws Throwable
    {
        ...
        assertEquals("Segment memory limiter should revert to zero after compaction.", 0L, getSegmentBufferUsedBytes(jmxConnection));
        assertEquals("There should be no segment builders in progress.", 0L, getColumnIndexBuildsInProgress(jmxConnection));
    }
    ...
    private void verifyTruncateWithIndex(boolean concurrentTruncate) throws Throwable
    {
        ...
        assertEquals("Segment memory limiter should revert to zero after truncate.", 0L, getSegmentBufferUsedBytes(jmxConnection));
        assertEquals("There should be no segment builders in progress.", 0L, getColumnIndexBuildsInProgress(jmxConnection));
    }
    ...
    public void verifyRebuildCorruptedFiles() throws Throwable
    {
        ...
        {
            verifyRebuildCorruptedFiles(corruptionType, false);
            verifyRebuildCorruptedFiles(corruptionType, true);
        }
    ...
    private void verifyRebuildIndexComponent(IndexComponents.IndexComponent component, CorruptionType corruptionType,
                                             boolean failedStringIndex, boolean failedNumericIndex, boolean rebuild) throws Throwable
    {
        // The completion markers are valid if they exist on the file system so we only need to test
        // their removal
        if (component.ndiType.completionMarker() && (corruptionType != CorruptionType.REMOVED))
            return;
    ...
    public void nodetoolStopInitialIndexBuild() throws Throwable
    {
        ...
        assertEquals("Segment memory limiter should revert to zero on interrupted compactions.", 0L, getSegmentBufferUsedBytes(jmxConnection));
        assertEquals("There should be no segment builders in progress.", 0L, getColumnIndexBuildsInProgress(jmxConnection));
