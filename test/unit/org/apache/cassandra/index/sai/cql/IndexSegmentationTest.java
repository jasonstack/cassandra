...
public class IndexSegmentationTest extends SAITester
{
    ...
    public void testWidePartition() throws Throwable
    {
        ...
        for (int lastValidSegmentRowId : Arrays.asList(999, 10001, 19999, 21000))
        {
            ...
            assertEquals(count + segments, getIndexFetchCount.getAsLong());
            ...
            count = getIndexFetchCount.getAsLong();
            assertRowCount(num, "SELECT id1 FROM %s WHERE v1>=0");
            assertEquals(count + segments * (pages + 1), getIndexFetchCount.getAsLong()); // last page has no result
    ...
    public void testSkinnyPartition() throws Throwable
    {
        ...
        for (int lastValidSegmentRowId : Arrays.asList(0, 1, 3, 9, 25, 30, 50, 60, 99, 101, 499, 999, 1001))
        {
            ...
            segments = (int) Math.ceil(num * 1.0 / (lastValidSegmentRowId + 1));
            verifySegmentSplit(v1IndexName, v2IndexName, num, segments, getIndexFetchCount);
        }
    }

    private void verifySegmentSplit(String v1IndexName, String v2IndexName, int num, int segments, LongSupplier getIndexFetchCount) throws Throwable
    {
        verifySegments(v1IndexName, segments);
        verifySegments(v2IndexName, segments);
        waitForAssert(() -> verifyIndexFiles(1, 1), WAIT_FOR_INDEX_FILE_CREATION);
        ...
        // verify select * query touches all segments
        count = getIndexFetchCount.getAsLong();
        assertRowCount(num, "SELECT id1 FROM %s WHERE v1>=0");
        assertEquals(count + segments, getIndexFetchCount.getAsLong());
