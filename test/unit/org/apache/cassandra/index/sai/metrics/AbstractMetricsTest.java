...
public abstract class AbstractMetricsTest extends SAITester
{
    ...
    protected long sumMetrics(ObjectName... names) throws Exception
    {
        long sum = 0;

        for (ObjectName name : names)
        {
            sum += ((Number) getMetricValue(name)).longValue();
        }
        return sum;
    }
    ...
    protected void waitForIndexCompaction(String keyspace, String table, String index)
    {
        waitForAssert(() -> {
            try
            {
                Assert.assertEquals(1L, getMetricValue(objectName("CompactionCount", keyspace, table, index, "IndexMetrics")));
            }
            catch (Throwable ex)
            {
