...
{
    private static final KeyFetcher KEY_FETCHER = new KeyFetcher() {
        @Override
    ...
    public void testEqQueriesAgainstStringIndex() throws Exception
    {
        ...
        try (IndexSearcher searcher = buildIndexAndOpenSearcher(numTerms, numPostings, termsEnum))
        {
            ...
            {
                try (RangeIterator results = searcher.search(new Expression("meh", UTF8Type.instance).add(Operator.EQ, termsEnum.get(t).left), SSTableQueryContext.forTest()))
                {
            ...
            RangeIterator results = searcher.search(new Expression("meh", UTF8Type.instance)
            ...
            results = searcher.search(new Expression("meh", UTF8Type.instance)
    ...
    public void testUnsupportedOperator() throws Exception
    {
        ...
        try (IndexSearcher searcher = buildIndexAndOpenSearcher(numTerms, numPostings, termsEnum))
        {
            searcher.search(new Expression("meh", UTF8Type.instance).add(Operator.GT, UTF8Type.instance.decompose("a")), SSTableQueryContext.forTest());
    ...
    private IndexSearcher buildIndexAndOpenSearcher(int terms, int postings, List<Pair<ByteBuffer, IntArrayList>> termsEnum) throws IOException
    {
        ...
        final SegmentMetadata segmentMetadata = new SegmentMetadata(0,
                ...
                termsEnum.get(0).left,
                termsEnum.get(terms - 1).left,
                indexMetas);
        ...
        try (SSTableIndex.PerIndexFiles indexFiles = new SSTableIndex.PerIndexFiles(indexComponents, true))
        {
            ...
            Segment segment = new Segment(factory, factory, KEY_FETCHER, indexFiles, segmentMetadata);
