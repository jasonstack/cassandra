...
{
    private static Injections.Counter INTERSECTION_FLOW_COUNTER = Injections.newCounter("IntersectionIteratorCounter")
            .add(InvokePointBuilder.newInvokePoint().onClass(RangeIntersectionIterator.AbstractIntersectionIterator.class).onMethod("<init>"))
            .build();

    private static Injections.Counter POSTINGS_READER_OPEN_COUNTER = Injections.newCounter("PostingsReaderOpenCounter")
            .add(InvokePointBuilder.newInvokePoint().onClass(PostingsReader.class).onMethod("<init>"))
            .build();

    private static Injections.Counter POSTINGS_READER_CLOSE_COUNTER = Injections.newCounter("PostingsReaderCloseCounter")
            .add(InvokePointBuilder.newInvokePoint().onClass(PostingsReader.class).onMethod("close"))
            .build();
    ...
    private static void setLimits(final int selectivityLimit) throws Throwable
    {
