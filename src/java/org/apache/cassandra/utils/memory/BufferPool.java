...
public class BufferPool
{
    ...
    @VisibleForTesting
    public void forEachGlobalChunk(Consumer<Chunk> consumer)
    {
        globalPool.chunks.forEach(consumer);
    }

    @VisibleForTesting
    public void forEachGlobalPartiallyFreedChunk(Consumer<Chunk> consumer)
    {
        globalPool.partiallyFreedChunks.forEach(consumer);
    }

    @VisibleForTesting
    public void unsafeSetMemoryUsageThreshold(long memoryUsageThreshold)
    {
        this.memoryUsageThreshold = memoryUsageThreshold;
    }
    ...
    final class GlobalPool implements Supplier<Chunk>, Recycler
    {
        ...
        private Chunk allocateMoreChunks()
        {
            while (true)
            {
                ...
                if (cur + MACRO_CHUNK_SIZE > memoryUsageThreshold)
                {
                    ...
                    {
                        noSpamLogger.info("{} pool maximum memory usage reached ({}), cannot allocate chunk of {}",
                                          name,
                                          prettyPrintMemory(memoryUsageThreshold),
        ...
        public void recycle(Chunk chunk)
        {
            ...
            if (debug != null)
            {
                ...
                assert !partiallyFreedChunks.contains(chunk);
    ...
    public final class LocalPool implements Recycler
    {
        ...
        private void put(ByteBuffer buffer, Chunk chunk)
        {
            ...
            if (chunk.recycler != null && chunk.recycler.canRecyclePartially())
                freeChunkWithPartialRecycle(chunk, owner, buffer);
            else
                freeChunk(chunk, owner, buffer);
        ...
        /**
         * Partially freed chunk will not be reused and causing fragmentation, because ChunkCache
         * can hold a buffer for arbitrary lifetime.
         */
        private void freeChunk(Chunk chunk, LocalPool owner, ByteBuffer buffer)
        {
        ...
        /**
         * Partially freed chunk will be recirculated into global list to improve utilization,
         * but fragmentation still exists due to different allocation sizes.
         */
        private void freeChunkWithPartialRecycle(Chunk chunk, LocalPool owner, ByteBuffer buffer)
        {
            assert chunk.recycler.canRecyclePartially();
            ...
            // * others:
            //          * if chunk is offline, aka evicted, try to bring it back online and recyle partially
            //          * if chunk is online, do nothing.
            if (free == 0L)
            {
                assert owner == this;
                // 0L => we own recycling responsibility, so must recycle;
            ...
            else if (owner == null && chunk.status() == Chunk.Status.OFFLINE && chunk.setOnline())
            {
    ...
    private final InfiniteLoopExecutor EXEC = new InfiniteLoopExecutor("LocalPool-Cleaner", this::cleanupOneReference).start();
    ...
    final static class Chunk
    {
         ...
         *
         * if all slots are allocated, freeSlots will be set to 0.
         */
