...
public class SSTableIndex
{
    ...
    public RangeIterator search(Expression expression, AbstractBounds<PartitionPosition> keyRange, SSTableQueryContext context)
    {
        ...
        for (Segment segment : segments)
        {
            ...
            {
                builder.add(segment.search(expression, context));
            }
        }
    ...
    public static class PerIndexFiles implements Closeable
    {
        ...
        public PerIndexFiles(IndexComponents components, boolean isStringIndex)
        {
            ...
            if (isStringIndex)
            {
                files.put(components.postingLists, components.createFileHandle(components.postingLists));
                files.put(components.termsData, components.createFileHandle(components.termsData));
            }
            else
            {
                files.put(components.kdTree, components.createFileHandle(components.kdTree));
                files.put(components.kdTreePostingLists, components.createFileHandle(components.kdTreePostingLists));
            }
        }
