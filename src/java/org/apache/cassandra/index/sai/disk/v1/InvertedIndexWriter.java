...
public class InvertedIndexWriter implements Closeable
{
    ...
    public InvertedIndexWriter(IndexComponents indexComponents) throws IOException
    {
        this.termsDictionaryWriter = new TrieTermsDictionaryWriter(indexComponents);
        this.postingsWriter = new PostingsWriter(indexComponents);
        compressionParams = indexComponents.getCompressionParams();
    }
     ...
     */
    public ComponentMetadataMap writeAll(TermsIterator terms) throws IOException
    {
        ...
        while (terms.hasNext())
        {
            ByteBuffer term = terms.next();
            try (PostingList postings = terms.postings())
            {
                postingsAdded += postings.size();

                final long offset = postingsWriter.write(postings);
                termsDictionaryWriter.add(ByteComparable.fixedLength(term), offset);
            }
        }
        ...
        ComponentMetadataMap components = new ComponentMetadataMap();
