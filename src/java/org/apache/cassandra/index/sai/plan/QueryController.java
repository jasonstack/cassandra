...
public class QueryController
{
    ...
    /**
     * @return token range used in the read command
     */
    DataRange dataRange()
    {
        return range;
     ...
     */
    public RangeIterator.Builder getIndexes(OperationType op, Collection<Expression> expressions)
    {
        ...
        Set<Map.Entry<Expression, Set<SSTableIndex>>> view = referenceAndGetView(op, expressions).entrySet();
        ...
        try
        {
            for (Map.Entry<Expression, Set<SSTableIndex>> e : view)
            {
                ...
                RangeIterator index = TermIterator.build(e.getKey(), e.getValue(), range.keyRange(), queryContext);
     ...
     */
    private Map<Expression, Set<SSTableIndex>> referenceAndGetView(OperationType op, Collection<Expression> expressions)
    {
        ...
        try
        {
            while (true)
            {
                ...
                Map<Expression, Set<SSTableIndex>> view = getView(op, expressions);
    ...
    /**
     * Returns the {@link DataRange} covered by the specified {@link ReadCommand}.
     *
     * @param command a read command
     * @return the data ranges covered by {@code command}
     */
    private static DataRange dataRange(ReadCommand command)
    {
        if (command instanceof SinglePartitionReadCommand)
        {
            SinglePartitionReadCommand cmd = (SinglePartitionReadCommand) command;
            DecoratedKey key = cmd.partitionKey();
            return new DataRange(new Range<>(key, key), cmd.clusteringIndexFilter());
        }
        else if (command instanceof PartitionRangeReadCommand)
        {
            PartitionRangeReadCommand cmd = (PartitionRangeReadCommand) command;
            return cmd.dataRange();
        }
        else
        {
